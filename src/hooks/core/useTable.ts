...
function useTableImpl<TApiFn extends (params: any) => Promise<any>>(
  ...
  const pageKey = paginationKey?.pageNumber || tableConfig.paginationKey.pageNumber
  ...
  const pagination = reactive<Api.Common.PaginationParams>({
    pageNumber: ((searchParams as Record<string, unknown>)[pageKey] as number) || 1,
    pageSize: ((searchParams as Record<string, unknown>)[sizeKey] as number) || 10,
    totalRow: 0
  })
    ...
    try {
      ...
      if (useCache && cache) {
        ...
        if (cachedItem) {
          ...
          if (paramsRecord[pageKey] !== pagination.pageNumber) {
            paramsRecord[pageKey] = pagination.pageNumber
          }
      ...
      if (err instanceof Error && err.message === '请求已取消') {
        ...
        return { records: [], totalRow: 0, pageNumber: 1, pageSize: 10 }
      }
  ...
  const handleCurrentChange = async (newCurrent: number): Promise<void> => {
    ...
    if (pagination.pageNumber === newCurrent) {
    ...
    try {
      ...
      // 修复：只更新必要的状态
      const paramsRecord = searchParams as Record<string, unknown>
      pagination.pageNumber = newCurrent
  ...
  const refreshRemove = async (): Promise<void> => {
    ...
    // 如果当前页为空且不是第一页，回到上一页
    if (data.value.length === 0 && pageNumber > 1) {
      pagination.pageNumber = pageNumber - 1
      ;(searchParams as Record<string, unknown>)[pageKey] = pageNumber - 1
      await getData()
    }
  }
